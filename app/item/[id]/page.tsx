...
import { GlobalMenu, IdentityButton } from "@/components/GlobalMenu";
...
export default function ItemPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function saveText() {
    await patch({ title: title.trim() || "Naamloos voorwerp", description: description.trim() || null });
  ...
  async function saveAndNext() {
    ...
    if (titleChanged || descriptionChanged) {
      await patch({ title: title.trim() || "Naamloos voorwerp", description: description.trim() || null }, false);
    }
    ...
    <main className="shell stack item-workbench">
      <header className="sticky-head workbench-head">
        <div className="workbench-head-row">
          ...
          <div className="workbench-head-actions">
            ...
            <GlobalMenu roomId={item.room_id} />
          </div>
        </div>
      </header>
      ...
      {photos.length > 0 ? (
        <section className="hero-gallery">
          <button type="button" className="hero-photo-button" onClick={() => setViewerOpen(true)} onTouchStart={(e) => setTouchStart(e.touches[0].clientX)} onTouchEnd={(e) => finishSwipe(e.changedTouches[0].clientX)} aria-label="Foto op volledig scherm bekijken">
              ...
              <div className="gallery-meta">
                <div className="gallery-dots">
                  {photos.map((p: any, index: number) => (
                    <button key={p.id} type="button" className={index === activePhoto ? "gallery-dot active" : "gallery-dot"} onClick={() => setActivePhoto(index)} aria-label={`Foto ${index + 1}`} />
                  ))}
                </div>
      ...
      <section className="item-identity card">
        {editingTitle ? (
          ...
          <button type="button" className="editable-title" onClick={() => setEditingTitle(true)}><span>{item.title}</span><span>✎</span></button>
        )}
        ...
        {editingDescription ? (
          ...
          <button type="button" className="editable-description" onClick={() => setEditingDescription(true)}><span>{item.description || "Voeg een omschrijving toe"}</span><span>✎</span></button>
        )}
      </section>
      ...
      <section className="card stack vote-section">
        ...
        <div className="vote-buttons">
          {(Object.keys(voteMeta) as VoteLevel[]).map((level) => (
            <button key={level} disabled={busy || !selected} className={`vote-button ${myVote === level ? "active" : ""}`} onClick={() => vote(level)}>
        ...
        {votes.length ? (
          <div className="people-votes">
            {(Object.keys(voteMeta) as VoteLevel[]).map((level) => groupedVotes[level].map((v: any) => (
              <span key={`${level}-${v.member_id}`} className={`person-chip ${level}`} title={v.members?.name || ""}>{voteMeta[level].icon} {v.members?.name || "?"}</span>
            )))}
          </div>
        ) : <div className="subtle">Nog niemand heeft gestemd.</div>}

        {decisionMakers.length ? (
          <details className="decision-makers-box compact-decision-makers">
            <summary><strong>Hoofd-beslissers</strong><span>{decisionMakerVotes.length}/{decisionMakers.length} ›</span></summary>
            <div className="people-votes">
              {decisionMakers.map((m) => {
                const v = votes.find((vote: any) => vote.member_id === m.id);
                return <span key={m.id} className="person-chip decision">{v ? "✅" : "⏳"} {m.name}{v ? ` ${voteMeta[v.level as VoteLevel]?.icon || ""}` : ""}</span>;
              })}
            </div>
          </details>
        ) : null}
      ...
      <section className="card stack destination-section">
        <div><strong>Bestemming</strong><div className="subtle" style={{ marginTop: 3 }}>Kies wat er uiteindelijk met dit voorwerp gebeurt.</div></div>
        <div className="destination-grid">
          {destinations.map((destination) => {
            const active = destination.value === "undecided" ? !item.destination : item.destination === destination.value;
            return <button key={destination.value} type="button" disabled={busy} className={`destination-chip ${active ? "active" : ""}`} onClick={() => chooseDestination(destination.value)}>{destination.label}</button>;
          })}
        </div>
        ...
        {item.destination === "family" ? (
          <div className="family-assignment">
            ...
            <div className="family-chip-grid">
              {members.map((m) => <button key={m.id} type="button" disabled={busy} className={`family-chip ${item.assigned_member_id === m.id ? "active" : ""}`} onClick={() => patch({ assigned_member_id: m.id, status: "decided" })}>{m.name}</button>)}
            </div>
          </div>
        ) : null}
      </section>
      ...
      <section className="card stack physical-section">
        <div><strong>Fysieke status</strong><div className="subtle" style={{ marginTop: 3 }}>Dit staat los van de gekozen bestemming.</div></div>
        <button className={`physical-toggle ${item.status === "removed" ? "removed" : "present"}`} disabled={busy} onClick={toggleRemoved}>
      ...
      <details className="card management-details">
        ...
        <div className="management-content stack">
          <div className="subtle">Kamer: <strong>{item.rooms?.name || "Onbekend"}</strong>{item.rooms?.floor ? ` · ${item.rooms.floor}` : ""}</div>
          {item.created_at ? <div className="subtle">Toegevoegd: {new Date(item.created_at).toLocaleDateString("nl-BE")}</div> : null}
          <label className="btn" style={{ textAlign: "center" }}>📷 Nog een foto toevoegen<input hidden type="file" accept="image/*" capture="environment" onChange={(e) => addPhoto(e.target.files?.[0] || null)} /></label>
      ...
      <div className="workbench-pager">
        ...
        <button className="save-next" disabled={busy} onClick={saveAndNext}>{next ? "Opslaan & volgende →" : "Opslaan & terug →"}</button>
      </div>
      ...
      {viewerOpen && photos[activePhoto] ? (
        <div className="photo-viewer" role="dialog" aria-modal="true" onClick={() => setViewerOpen(false)}>
          ...
          {photos.length > 1 ? <div className="viewer-controls" onClick={(e) => e.stopPropagation()}><button onClick={() => movePhoto(-1)}>←</button><span>{activePhoto + 1} / {photos.length}</span><button onClick={() => movePhoto(1)}>→</button></div> : null}
        </div>
      ) : null}
